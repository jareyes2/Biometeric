...
def plot_scoreDist(gen_scores, imp_scores, plot_title):
    ...
    plt.show()
    fig.savefig(plot_title.split(",")[0] + "_SCOREDIST.png", bbox_inches="tight")
    plt.close()
    return
...
def plot_det(far, frr, plot_title):
    ...
    plt.xlabel('FAR')
    plt.ylabel('FRR')
    plt.title(plot_title.split(",")[0] + '\nEER = %.3f' % eer)
    plt.show()
    fig.savefig(plot_title.split(",")[0] + "_DET.png", bbox_inches="tight")
    plt.close()
    return eer
...
def plot_roc(far, tpr, plot_title):
    ...
    plt.title(plot_title)
    plt.show()
    fig.savefig(plot_title.split(",")[0] + "_ROC.png", bbox_inches="tight")
    plt.close()
    return
...
def perf(gen_scores, imp_scores, session1, session2):
    far, frr, tpr = compute_rates(gen_scores, imp_scores, 200)    
    plot_title = 'Session %d vs %d, GenMean=%0.2f, GenStd=%0.2f\nImpMean=%0.2f, ImpStd=%0.2f' % (session1, session2, np.mean(gen_scores), 
                                                                               np.std(gen_scores), 
                                                                               np.mean(imp_scores), 
                                                                               np.std(imp_scores))
    plot_scoreDist(gen_scores, imp_scores, plot_title)
    plot_roc(far, tpr, plot_title)
    eer = plot_det(far, frr, plot_title)   
